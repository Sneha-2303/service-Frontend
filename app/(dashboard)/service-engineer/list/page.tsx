...
export default function ServiceEngineerListPage() {
  ...
  const fetchEngineers = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/service-engineers`);
      if (!response.ok) throw new Error("Failed to fetch engineers");
      const data = await response.json();
      
      const mappedData: ServiceEngineer[] = data.map((e: any, index: number) => {
        const names = (e.full_name || "").split(" ");
        return {
          id: e.id,
          srNo: index + 1,
          initials: (e.full_name || "").split(" ").map((n: string) => n[0]).join("").toUpperCase(),
          name: e.full_name,
          firstName: names[0] || "",
          ...
          regDate: new Date(e.created_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
          assignedAction: e.is_under_dealer ? "Assigned Under Dealer" : "Not Assigned",
          isTeamLead: e.is_team_lead || false,
          isUnderDealer: e.is_under_dealer || false
        };
      });
      ...
      alert("Failed to fetch engineers from server");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddEngineer = async () => {
    ...
    const payload = {
      ...
      email: formData.email || `${formData.mobile}@mitra.com`,
      ...
      is_under_dealer: formData.isUnderDealer,
      status: "Active"
    };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || "Failed to add engineer");
      }
  ...
  const openEditModal = (engineer: ServiceEngineer) => {
    ...
    setAvailableDistricts(engineer.state ? getDistricts([engineer.state]) : []);
    setAvailableTalukas(engineer.district ? getTalukas([engineer.district]) : []);
  ...
  const dynamicTeamLeads = [...new Set(engineers.filter(e => e.isTeamLead).map(e => e.name))];
