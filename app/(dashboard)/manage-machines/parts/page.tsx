...
export default function MachinePartsPage() {
  ...
  const [availableModels, setAvailableModels] = useState<MachineModel[]>([]);
  ...
  const [formData, setFormData] = useState({
    ...
    machineModelId: "" as string | number
  });
  ...
  const fetchInitialData = async () => {
    setLoading(true);
    try {
      const [partsData, machinesData, modelsData] = await Promise.all([
        getParts(),
        getMachines(),
        getAllMachineModels()
      ]);
      ...
      const formattedParts = partsData.map((p: any, index: number) => ({
        id: p.id,
        srNo: index + 1,
        ...
        machineModelId: p.machine_model_id,
        machineName: p.machine_name || "N/A",
        machineModel: p.machine_model_name || "N/A",
  ...
  const handleInputChange = (field: string, value: any) => {
    ...
    // Update available models when machine changes
    if (field === "machineId") {
      const filteredModels = allModels.filter(m => m.machine_id === Number(value));
      setAvailableModels(filteredModels);
      setFormData(prev => ({ ...prev, machineModelId: "" }));
    }
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // For now, we don't have a file upload API for parts, 
      // so we'll just use the preview as a fake URL or wait for user to provide a URL
  ...
  const handleAddPart = async () => {
    ...
    try {
      await createPart({
        ...
        machine_id: formData.machineId ? Number(formData.machineId) : null,
        machine_model_id: formData.machineModelId ? Number(formData.machineModelId) : null
      });
  ...
  const handleEditPart = async () => {
    ...
    try {
      ...
      alert(error.message || "Failed to update part");
    }
  };
  ...
  const openEditModal = (part: MachinePart) => {
    ...
    setFormData({
      ...
      machineModelId: part.machineModelId || ""
    });
    setPreviewImage(part.photo);
    
    if (part.machineId) {
      setAvailableModels(allModels.filter(m => m.machine_id === part.machineId));
    } else {
      setAvailableModels([]);
    }
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-gray-500">
                      {formData.photo.startsWith('data:') ? "New file selected" : "No new file chosen"}
                    </span>
                  </div>
                  <p className="text-xs text-gray-400 mt-1">
                    only jpg, png, jpeg files will be accepted
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            ...
            <div className="p-6 text-center">
              ...
              <p className="text-black mb-2">Are you sure you want to delete this part?</p>
              <p className="font-bold text-black">{selectedPart.partName}</p>
