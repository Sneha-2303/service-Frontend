...
export default function CustomerPage() {
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      const transformedCustomers = data.map((c: any, index: number) => ({
        id: c.id,
        srNo: index + 1,
        customerName: c.full_name || `${c.first_name || ''} ${c.last_name || ''}`.trim(),
        ...
        address: c.address || "",
        machine: c.machine || "",
        machineModel: c.machine_model || "",
        warranty: "Under Warranty",
        serialNo: c.serial_no || "",
        status: "Active",
        regDate: c.created_at?.split('T')[0] || new Date().toISOString().split('T')[0]
      }));
  ...
  const handleAddCustomer = async () => {
    ...
    try {
      await createCustomer({
        ...
        middle_name: addFormData.middleName || null,
        last_name: addFormData.lastName,
        mobile: addFormData.mobile,
        email: null,
        address: addFormData.address || null,
        village: addFormData.village || null,
        state_name: addFormData.state,
        district_name: addFormData.district,
        taluka_name: addFormData.taluka,
        pincode: null
      });
  ...
  const handleEditCustomer = async () => {
    ...
    try {
      await updateCustomer(selectedCustomer.id, {
        ...
        middle_name: editFormData.middleName || null,
