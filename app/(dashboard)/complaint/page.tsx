...
import { 
  ...
  getComplaintSubcategoriesByCategory,
  getCustomers
} from "@/lib/api";
...
export default function ComplaintPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      // Fetch everything
      const [comps, cats, custs] = await Promise.all([
        getComplaints().catch(() => []),
      ...
      const formattedComplaints = comps.map((c: any, index: number) => ({
        id: c.id,
        srNo: index + 1,
        complaintsId: c.complaint_id,
        customerName: c.customer_name || "Unknown",
        mobile: c.customer_mobile || "Unknown",
        problem: c.problem_description,
        status: c.status,
        installationDate: c.installation_date || new Date().toISOString(),
        warrantyDate: new Date().toISOString(), // Mock warranty
        seName: c.assigned_to || "Unassigned",
        date: c.created_at || new Date().toISOString(),
        updateSE: c.assigned_to || "Unassigned",
        otp: "123456",
        machineName: c.machine_name || "Unknown",
        machineModel: c.machine_model || "Unknown",
        machineSrNo: "Unknown",
        category: "Unknown",
        subCategory: "Unknown",
        subCategory2: "Unknown"
      }));
  ...
  const handleInputChange = async (field: string, value: any) => {
    ...
    if (field === "categoryId") {
      try {
        const subcats = await getComplaintSubcategoriesByCategory(parseInt(value));
    ...
    if (field === "customerId") {
      const selectedCustomer = dbCustomers.find(c => c.id.toString() === value);
      if (selectedCustomer) {
        setFormData(prev => ({ 
          ...prev, 
          customerId: value,
          machineName: "Machine Loading...", // Adjust if customer machines available
          machineId: "1" // Default mock machine ID since we need it for backend
        }));
      }
    }
  };
  ...
  const handleAddComplaint = async () => {
    if (!formData.customerId || !formData.machineId || !formData.description) {
