...
import { getDealers, createDealer, updateDealer, deleteDealer } from '@/lib/api';
...
export default function DealerPage() {
  ...
  const { getStates, getDistricts, getTalukas, loading: locLoading } = useLocations();
  ...
  // Available districts/talukas based on selected states, always including the currently selected ones
  const availableDistricts = Array.from(new Set([
    ...fDistricts,
    ...getDistricts(fStates)
  ]));
  
  const availableTalukas = Array.from(new Set([
    ...fTalukas,
    ...getTalukas(fDistricts)
  ]));
  ...
  const openEdit = (d: Dealer) => {
    ...
    setFStates(d.state ? [d.state] : []);
    setFDistricts(d.districts || []);
    setFTalukas(d.talukas || []);
    setFMobiles(d.mobile || "");
    setFEmails(d.email || "");
    setFError(""); setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        await updateDealer(editId, {
          name: fName,
          contact_person: fName,
          mobile: fMobiles || "9999999999",
          email: fEmails || "dealer@example.com",
          address: fAddress,
          state_name: fStates[0],
          district_name: fDistricts[0],
          taluka_name: fTalukas[0]
        });
      } else {
        await createDealer({
          name: fName,
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-6 pb-4 flex-wrap">
        ...
        <button onClick={openAdd}
          ...
          style={{ background: "#2e7d32", boxShadow: "0 4px 14px rgba(46,125,50,0.35)" }}>
          <Plus size={16} /> Add
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center"
          ...
          <div className="relative w-full max-w-2xl mx-4 rounded-3xl overflow-hidden shadow-2xl"
            ...
            <div className="flex items-center justify-between px-7 py-5"
              style={{ background: "#2e7d32", borderBottom: "1px solid #c8e6c9" }}>
              <div>
            ...
            <form onSubmit={handleSubmit} className="px-7 py-6 space-y-5">
              ...
              <div>
                ...
                <textarea value={fAddress} onChange={e => setFAddress(e.target.value)}
                  ...
                  onFocus={e => (e.target as HTMLTextAreaElement).style.borderColor = "#2e7d32"}
              ...
              <div>
                ...
                <MultiSelect
                  label="states" options={getStates()}
                  selected={fStates}
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit"
                  ...
                  style={{ background: "#2e7d32", boxShadow: "0 4px 14px rgba(46,125,50,0.3)" }}>
                  <Plus size={16} />
