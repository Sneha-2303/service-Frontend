...
export default function CollectionPage() {
  ...
  const [serviceEngineers, setServiceEngineers] = useState<string[]>([]);
  ...
  const fetchEngineers = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const names = data.map((eng: any) => eng.full_name);
        setServiceEngineers(["All Engineers", ...names]);
      }
    } catch (error) {
  ...
  const applyFilters = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(c => 
        ...
        c.customer_mobile.includes(searchTerm) ||
        c.se_name.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    
    if (selectedEngineer && selectedEngineer !== "All Engineers") {
      filtered = filtered.filter(c => c.se_name === selectedEngineer);
    }
  ...
  const exportToCSV = () => {
    ...
    const csvData = filteredCollections.map((c, index) => [
      c.ticket_id,
      index + 1,
      c.customer_name,
      c.customer_mobile,
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">
                ₹{filteredCollections.reduce((sum, c) => sum + c.total_amt, 0).toLocaleString('en-IN')}
              </p>
            </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                currentRecords.map((collection, index) => (
