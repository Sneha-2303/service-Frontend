  ...
  const applyFilters = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(r => 
        r.complaint_id.toLowerCase().includes(searchTerm.toLowerCase()) ||
        r.customer_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        r.service_engineer_name.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    
    if (selectedEngineer && selectedEngineer !== "All Engineers") {
      filtered = filtered.filter(r => r.service_engineer_name === selectedEngineer);
    }
    
    if (fromDate) {
      filtered = filtered.filter(r => r.created_at >= fromDate);
    }
    
    if (toDate) {
      filtered = filtered.filter(r => r.created_at <= toDate);
    }
