...
import { getTalukas, createTaluka, updateTaluka, deleteTaluka } from "@/lib/api";
...
const getZoneForState = (state: string): string => {
  ...
  return stateZones[state] || "West";
};

export default function CityManagementPage() {
  const { hierarchy, loading: locationsLoading } = useLocations();
  ...
  const fetchCities = async () => {
    ...
    try {
      ...
      const mappedCities: City[] = data.map((t: any, index: number) => ({
        id: t.id,
        srNo: index + 1,
        name: t.name,
        code: t.code || "N/A",
  ...
  const handleInputChange = (field: string, value: any) => {
    ...
    if (field === "stateId") {
      const state = hierarchy.find(s => s.id === Number(value));
      const zone = state ? getZoneForState(state.name) : "";
      setFormData(prev => ({ ...prev, districtId: "", zone }));
    }
  };
  ...
  const handleAddCity = async () => {
    if (!formData.name || !formData.code || !formData.districtId || !formData.pincode) {
  ...
  const handleEditCity = async () => {
    ...
    setIsSubmitting(true);
    try {
      await updateTaluka(selectedCity.id, {
        name: formData.name,
        code: formData.code.toUpperCase(),
        district_id: Number(formData.districtId),
        pincode: formData.pincode,
  ...
  const handleDeleteCity = async () => {
    ...
    setIsSubmitting(true);
    try {
      await deleteTaluka(selectedCity.id);
      await fetchCities();
  ...
  const openEditModal = (city: City) => {
    ...
    for (const state of hierarchy) {
      if (state.name === city.state) {
        ...
        const district = state.districts.find(d => d.name === city.district);
        if (district) {
  ...
  const uniqueDistricts = filters.state 
    ? [...new Set(cities.filter(c => c.state === filters.state).map(c => c.district))]
    : [];
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {hierarchy.find(s => s.id === Number(formData.stateId))?.districts.map(district => (
