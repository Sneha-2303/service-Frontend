...
export default function RegionManagementPage() {
  ...
  const fetchStates = async () => {
    ...
    try {
      ...
      // Map backend data to frontend State interface
      // Since backend only has id and name, we mock the rest for now as per constraints
      const mappedStates: State[] = data.map((state: any, index: number) => ({
        id: state.id,
        srNo: index + 1,
        name: state.name,
        code: state.name.substring(0, 2).toUpperCase(),
        zone: "Central", // Default mock
        mttr: 0,         // Default mock
        status: "Active",
  ...
  const handleAddState = async () => {
    ...
    try {
      ...
      alert("Failed to add state. Please check if the state already exists.");
    }
  };
