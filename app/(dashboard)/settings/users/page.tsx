...
export default function UserManagementPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const mappedUsers: User[] = data.map((u: any, index: number) => {
  ...
  const handleAddUser = async () => {
    if (!formData.username || !formData.role || !formData.firstName || !formData.email || !formData.mobile || !formData.password) {
  ...
  const openEditModal = (user: User) => {
    ...
    // Find the original user to get the username
    // Since we don't store username in the 'User' interface, we might need it for update
    // But our updated updateUser API uses ID, so we are good.
    
    setFormData({
      username: "", // We don't allow changing username in this UI
      role: user.role,
