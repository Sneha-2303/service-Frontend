...
// We will fetch states dynamically
...
export default function DistrictManagementPage() {
  ...
  const fetchDistricts = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/locations/hierarchy`);
      if (!response.ok) throw new Error("Failed to fetch districts hierarchy");
      ...
      data.forEach((state: any) => {
        states.push({ id: state.id, name: state.name });
        if (state.districts) {
          state.districts.forEach((district: any) => {
            allDistricts.push({
              ...
              srNo: srNoCounter++,
              name: district.name,
              code: district.name.substring(0, 2).toUpperCase(), // Mock code
              state: state.name,
              zone: "Central", // Mock zone
              talukas: district.talukas ? district.talukas.length : 0,
              status: "Active",
      ...
      setStatesList(mockDistricts.map(d => ({id: 0, name: d.state})));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddDistrict = async () => {
    ...
    if (!stateObj) {
      alert("Selected state is invalid");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to add district");
      ...
      alert("Failed to add district. Please check if the district already exists.");
    }
  };
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    <option value="">Select State</option>
                    {statesList.map(state => (
                      <option key={state.id || state.name} value={state.name}>{state.name}</option>
                    ))}
                  </select>
                </div>
