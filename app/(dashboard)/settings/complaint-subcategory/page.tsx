...
import { 
  ...
  createComplaintSubcategory,
  updateComplaintSubcategory,
  deleteComplaintSubcategory
} from "@/lib/api";
...
export default function ComplaintSubCategoryPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const formattedSubcategories = subcategoriesData.map((s: any, index: number) => {
        const category = formattedCategories.find((c: any) => c.id === s.category_id);
        return {
          id: s.id,
          srNo: index + 1,
  ...
  const handleAddSubCategory = async () => {
    ...
    try {
      await createComplaintSubcategory({
  ...
  const handleEditSubCategory = async () => {
    ...
    try {
      await updateComplaintSubcategory(selectedSubCategory.id, {
        category_id: parseInt(formData.categoryId),
        name: formData.subCategoryName,
        status: formData.status
      });
  ...
  const handleDeleteSubCategory = async () => {
    ...
    try {
      await deleteComplaintSubcategory(selectedSubCategory.id);
