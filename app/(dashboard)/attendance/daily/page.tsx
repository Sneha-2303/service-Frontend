...
export default function DailyAttendancePage() {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      const mappedData: Attendance[] = data.map((item: any, index: number) => ({
        id: item.id,
        srNo: index + 1,
        name: item.user_name,
        date: item.date.split('T')[0],
        punchIn: item.punch_in ? "AM" : "N/A",
        punchInTime: item.punch_in ? item.punch_in.split('T')[1].split('.')[0] : "",
        punchInLocation: item.punch_in_location || "",
        punchInAddress: item.punch_in_location || "N/A",
        punchOut: item.punch_out ? "PM" : "N/A",
        punchOutTime: item.punch_out ? item.punch_out.split('T')[1].split('.')[0] : "",
        punchOutLocation: item.punch_out_location || "",
        punchOutAddress: item.punch_out_location || "N/A",
        hours: item.hours_worked || 0,
        activities: item.activities || 0
      }));

      setAttendance(mappedData);
      setFilteredAttendance(mappedData);

      // Fetch service engineers for filter
      const engRes = await fetch(`${API_BASE}/service-engineers`);
      if (engRes.ok) {
        const engData = await engRes.json();
        setServiceEngineers(["All Engineers", ...engData.map((e: any) => e.full_name)]);
      }
    } catch (error) {
