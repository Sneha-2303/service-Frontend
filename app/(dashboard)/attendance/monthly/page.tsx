...
export default function MonthlyAttendancePage() {
  ...
  const fetchAttendance = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/attendance/monthly?month=${selectedMonth}&year=${selectedYear}`);
      if (!res.ok) throw new Error("Failed to fetch monthly attendance");
      const data = await res.json();
      
      const mappedData: MonthlyAttendance[] = data.map((item: any, index: number) => ({
        id: item.user_id,
        srNo: index + 1,
        name: item.name,
        ...
        avgHoursPerDay: item.avg_hours_per_day,
