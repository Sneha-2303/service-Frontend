...
interface DealerReport {
  id: number;
  dealer_name: string;
  taluka_region: string;
  asm_rm_name: string;
  service_engineers: string[];
  uw_visit: number;
  gw_visit: number;
  ow_visit: number;
  total_visits: number;
...
export default function DealerReportPage() {
  ...
  const [asmRmNames, setAsmRmNames] = useState<string[]>(["All", "N/A"]);
  ...
  const fetchEngineers = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setServiceEngineersList(["All Engineers", ...data.map((eng: any) => eng.full_name)]);
      }
    } catch (error) {
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/reports/dealer`);
  ...
  const applyFilters = () => {
    ...
    if (filters.talukaRegion) {
      filtered = filtered.filter(r => r.taluka_region.toLowerCase().includes(filters.talukaRegion.toLowerCase()));
    }
    if (filters.asmRmName && filters.asmRmName !== "All") {
      filtered = filtered.filter(r => r.asm_rm_name === filters.asmRmName);
    }
    if (filters.serviceEngineer && filters.serviceEngineer !== "All Engineers") {
      filtered = filtered.filter(r => r.service_engineers.includes(filters.serviceEngineer));
    }
    if (searchTerm) {
      filtered = filtered.filter(r => 
        r.dealer_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        r.customer_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        r.service_engineers.some(se => se.toLowerCase().includes(searchTerm.toLowerCase())) ||
        r.complaint_ids.some(id => id.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
  ...
  const uniqueTalukas = [...new Set(reports.map(r => r.taluka_region))];
                  ...
                  <tr key={report.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-3 py-3 text-sm text-black">
                      <div className="flex flex-wrap gap-1">
                        {report.service_engineers.map((se, idx) => (
